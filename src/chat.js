...
function systemPrompt(mode = 'agent') {
  ...
  return [
    ...
    '- To email MANY leads, ALWAYS use ONE `outreach` command (it writes a single template and merge-renders it for every lead — `--limit N`, segment with `--where field~value`). NEVER emit one `email send` per lead — that does not scale and burns tokens. Approve the batch with `approve --all`.',
